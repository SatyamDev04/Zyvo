...
class ChatVC: UIViewController, UITextViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        if favoriteStatus == 1 {
            btnFavourite.setImage(UIImage(named: "FilledStars"), for: .normal)
        } else {
...
//        } else if self.isBlockStatus == 2 {
//            self.showAlert(for: "You are blocked by \(self.hostName)")
//        } else {
...
//            self.txtChat.text = "" }
        ...
        if self.txtChat.text == "" {
                ...
                sendMessage(inputMessage: txtChat.text!)
                self.txtChat.text = ""
                self.txtChat.textColor = .lightGray
                self.txtChat.text = placeholderText
                self.txtChat.becomeFirstResponder() // 👈 Keep keyboard open
            }
            ...
            .sink { [weak self] result in
                ...
                result?.handle(success: { response in
                    ...
                    if (isfavStatus) == "0" {
                        ...
                        self.btnFavourite.setImage(UIImage(named: "FilledStars"), for: .normal)
                    }
...
extension ChatVC : UITableViewDelegate,UITableViewDataSource {
    ...
    func addMessages(newMessages:Set<TCHMessage>,loadToBottom:Bool = true) {
        ...
        DispatchQueue.main.async {
            self.tableView!.reloadData()
            self.scrollToBottom()
          //  self.tableView.scrollToRow(at: IndexPath(row: self.sortedMessages.count - 1, section: 0), at: .bottom, animated: true)
        }
    }
    func sortMessages() {
